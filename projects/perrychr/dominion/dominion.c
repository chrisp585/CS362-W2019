...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus) {
  ...
  //*********************call refactored cards*********************************
  if (card == smithy){
	   smithyFunc(handPos, state);
	   return 0;
  }
  ...
  {
	  minionFunc(choice1, choice2, state, handPos);
	  return 0;
  }
  else if (card == baron)
  {
	  baronFunc(choice1, state);
	  return 0;
  }
  else
  {
...
int smithyFunc(int handPos, struct gameState *state)
{
	int currentPlayer = whoseTurn(state);
	//printf("Smithy Funtion\n");  //To test function hit
	//+3 Cards
	for (int i = 0; i < 4; i++) //****Bug: draws 4 cards instead of 3****
	{
		drawCard(currentPlayer, state);
	}
...
int adventurerFunc(struct gameState *state)
{
	...
	int cardDrawn;
	int z = 0;// this is the counter for the temp hand
	//printf("Adventurer Funtion\n");  //To test function hit
	while (drawntreasure < 4) {  //****Bug: counter starts at 1 instead of 0 ****
		if (state->deckCount[currentPlayer] < 1) {//if the deck is empty we need to shuffle discard and add to deck
			shuffle(currentPlayer, state);
		}
		drawCard(currentPlayer, state);
...
int sea_hagFunc(struct gameState *state)
{
	int currentPlayer = whoseTurn(state);
	...
	for (int i = 0; i < state->numPlayers; i++) {
		if (i != currentPlayer) {
			state->discard[i][state->discardCount[i]] = state->deck[i][state->deckCount[i]--];			    state->deckCount[i]--;
			state->discardCount[i]++;
			state->deck[i][state->deckCount[i]--] = province;//Top card now a curse  ***Makes the top card a province instead of a curse
		}
	}
	return 0;
}
...
int baronFunc(int choice1, struct gameState *state)
{
	int currentPlayer = whoseTurn(state);
